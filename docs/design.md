...
- **Journal is a rendered view; transcripts are the durable record.**
  `memo rebuild` regenerates the journal from transcript JSON.
...
memo open            open today's journal (or --dir) with $EDITOR / `open`
